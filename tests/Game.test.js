...
class TestStrategyWithPredictedMoves {
    constructor(fifoMoves, playerMarker) {
...
function createPlayerWithMoves(moves, playerName) {
    return new Player_1.Player(playerName, new TestStrategyWithPredictedMoves(moves, playerName));
}
describe('Tic-tac-toe game should work', () => {
    ...
    test('Player cannot make a move which is outside of the board (vertically)', () => {
        ...
        expect(game.displayGameState()).toBe(`It is player ${expectedPlayerName}'s (playing with ${Game_1.PLAYER_MARKER_X}) turn`);
    });
    ...
    test('Not allowed to make a move on a square which already is not empty,', () => {
        ...
        expect(() => game.play()).toThrow(new Error(`Player ${Game_1.PLAYER_MARKER_O} wants to play move ${sameMove}. This move is already taken on the board.`));
    });
    ...
    test('The game can be won vertically', () => {
        ...
        const expectedOutput = `Player ${winnerName} (${Game_1.PLAYER_MARKER_X}) has won!`;
        game.play();
        expect(game.displayGameState()).toBe(expectedOutput);
    });
    ...
    test('The game can be won diagonally (topLeft to bottomRight)', () => {
        ...
        const expectedOutput = `Player ${winnerName} (${Game_1.PLAYER_MARKER_O}) has won!`;
        game.play();
        expect(game.displayGameState()).toBe(expectedOutput);
    });
